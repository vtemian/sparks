...
def test_no_url_is_left_unset_rather_than_guessed() -> None:
    ...
    assert build_parser().parse_args(["--", "true"]).url is None
    assert build_parser().parse_args(["--url", "", "--", "true"]).url == ""
...
def test_run_takes_a_name_and_a_command_after_the_separator() -> None:
    args = build_parser().parse_args(["--name", "e0", "--", "python", "t.py"])
    assert args.name == "e0"
...
def test_run_refuses_an_empty_command() -> None:
    with pytest.raises(SystemExit):
        build_parser().parse_args(["--name", "e0", "--"])
...
def test_the_shared_dir_is_not_guessed_either() -> None:
    ...
    assert build_parser().parse_args(["--", "true"]).shared_dir is None
